...
public abstract class AbstractSqlHelper extends SQLiteOpenHelper {
	...
	public synchronized void releaseDb(boolean close, Object applicant) {
		...
		if (applicant == null)
			applicant = "unknown";
		if (acquireCounter == 0){
			if (Utils.DEBUG) Log.d(Utils.LOG_TAG, "database released by " + applicant.toString());
			keepDbOpen = false;
			...
			if (Utils.DEBUG) Log.d(Utils.LOG_TAG, String.format("request from %s: can NOT release db, still acquired... %d", applicant.toString(), acquireCounter));
			closeDb |= close;
		}
	}
}